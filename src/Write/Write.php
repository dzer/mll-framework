...
class Write
{
     ...
     * @return bool
     */
    public function checkTranscation()
    {
        if (empty($this)
     ...
     * @return array
     */
    public function batchUpdate($table, $data, $where, $transactionNum = '')
    {
        ...
        return array(
            'error' => isset($result['code']) && $result['code'] == 0 && $result['description'] == 'Succeed' ? 0 : 1,
            'msg' => $result['results'],
        );
    }
     ...
     * @return array
     */
    public function query($serviceName, $data, $type, $where = array())
    {
        ...
        $ret = self::ruleCore($type, $serviceName, self::whereChangeForRule($where), $data);
        if ($ret['code'] != 0) {
     ...
     * @return array
     */
    public function ruleCore($type, $service, $conditions = array(), $data = array())
    {
        $type = strtolower($type);
        $type_config = array(
            'insert' => 'C',
        ...
        if ($type == 'insert' || $type == 'batch_insert') {
            $error = '';
            if (!ErpTableIdHelper::detectAndHandleErpTable($service, $data, $type, $error)) {
     ...
     * @return array
     */
    public function whereChangeForRule($where)
    {
        if (empty($where)) {
            return array();
        }
     ...
     * @return array
     */
    public function updateTableById($table_name = '', $data = array(), $id = 0, $id_name = 'id', $obj = '')
    {
     ...
     * @return array
     */
    public function updateTableByIdAr($table_name = '', $data = array(), $id_ar = array(), $id_name = 'id', $obj = '')
    {
        if (empty($table_name)
            || empty($data)
            || empty($id_ar)
