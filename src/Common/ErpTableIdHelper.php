...
class ErpTableIdHelper
{
     ...
     * @param string $error
     *            错误信息提示会被写入该传入的变量
     * @return int 下一个可用于插入的存储主键ID， -1 表示不支持该表或未获得
     */
    public static function getERPTableNextPrimaryId($ERP_real_tablename, &$error = '')
    {
        ...
        $next_id = self::getNextId($seq, $error);
        return $next_id;
    }
     ...
     */
    public static function detectAndHandleErpTable($service_name, &$data, $type, &$error = '')
    {
            ...
            if (strtolower($type) == 'batch_insert') {
                ...
                foreach ($data as &$inner_data) {
                    ...
                    if (empty ($next_id) || $next_id == -1) {
                        $error .= " 自动设置ERP表主键时，规则{$rule_name}返回异常 ";
                        return false;
                    } else {
    ...
    /**
     * next available id for this table
     * @param string $service_name | table name
     * @param string $error
     * @return integer next available id for this table
     */
    private static function getNextId($service_name, &$error = '')
    {
        $try_time = 2;
        //$next_id = - 1;
        $rule_name = self::$NEXT_ID_RULE_NAME;
        ...
        do {
            $rule_res = Rule::callRule($rule_name, $param);
            if (empty ($rule_res)) {
                $error .= "规则{$rule_name}返回为空";
            }
